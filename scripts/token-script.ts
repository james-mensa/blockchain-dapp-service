...
async function Transfer(signer: number, value: any) {
  try {
    ...
    const etherValue = ethers.utils.parseEther(`${20}`);
    await Token.transfer(
      "0x0eB16E1209472a72213889D7cBC50df6f743fD54",etherValue
    );
...
async function SendEth(signer: number, value: number, _to?: any) {
  try {
    ...
    await Token.send("0x8d1cc9ae7b8d263976bbf4c263864915b86926b1", {
      value: etherValue,
    });
...
async function readDeploymentInfo(signer?: number) {
  let token = null;
  const deploymentPath = path.resolve(__dirname, "../token.json");
  ...
  try {
    ...
    token =
      signer !== undefined
        ? new hre.ethers.Contract(address, ERC20, AllSigners[signer!], value)
        : new hre.ethers.Contract(address, ERC20, value);
